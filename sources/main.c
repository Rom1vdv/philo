...
/*   Updated: 2023/05/25 18:25:35 by romvan-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_setzero(philo_datas, sizeof(philo_datas));
	...
	ft_setzero(philo_datas->forks, sizeof(*philo_datas->forks)
		* philo_datas->number_of_philos);
...
{	
	...
	if (ac != 5 && ac != 6)
		return (error_handling(ERROR_ARG_AMOUNT, NULL, NULL));
	array_of_philos = NULL;
	error_status = validate_input_args(av);
	if (error_status == ERROR_ARG_VALIDITY)
		return (error_handling(error_status, NULL, NULL));
	philo_datas = malloc(sizeof(*philo_datas));
	if (!philo_datas)
		return (error_handling(ERROR_ALLOCATION, NULL, NULL));
	error_status = init_datas(av, ac, philo_datas);
	if (error_status == ERROR_ALLOCATION)
		return (error_handling(error_status, philo_datas, NULL));
	error_status = init_philos(philo_datas, &array_of_philos);
	if (error_status == ERROR_ALLOCATION)
		return (error_handling(error_status, philo_datas, array_of_philos));
